...
namespace Portals.Examples {
    public class ThirdPersonCamera : PivotBasedCameraRig {
        [SerializeField] private float m_PortalClampSpeed = 10.0f;
        [SerializeField, Range(0, 1)] private float m_PortalRigidity = 0.5f;
        ...
        private Stack<Portal> m_PortalStack;
        ...
        private void OnTargetTeleported(Teleportable sender, Portal portal) {
            if (m_PortalStack.Count > 0 && portal.ExitPortal == m_PortalStack.Peek()) {
                m_PortalStack.Pop();
            } else {
                m_PortalStack.Push(portal);
            }
        }
        ...
        protected override void FollowTarget(float deltaTime) {
            ...
            foreach (Portal portal in m_PortalStack) {
                targetPosition = portal.InverseTeleportPoint(targetPosition);
            }

            Vector3 oldCameraPos = m_Cam.position;

            // camera position moves towards target position:
            transform.position = Vector3.Lerp(transform.position, targetPosition, deltaTime * m_MoveSpeed);

            Vector3 newCameraPos = m_Cam.position;

            Portal topPortal = m_PortalStack.Count > 0 ? m_PortalStack.Peek() : null;
            if (topPortal) {
                if (!topPortal.Plane.GetSide(oldCameraPos) && topPortal.Plane.GetSide(newCameraPos)) {
                    transform.position = topPortal.TeleportPoint(transform.position);
                    m_PortalStack.Pop();
                }
            }
            ...
            if (topPortal) {
                ClampPortalFrustum(topPortal);
            } else {
                //transform.rotation = Quaternion.Lerp(transform.rotation, rollRotation, m_TurnSpeed * m_CurrentTurnAmount * deltaTime);
                //m_Pivot.localRotation = Quaternion.Lerp(m_Pivot.localRotation, Quaternion.identity, m_TurnSpeed * m_CurrentTurnAmount * deltaTime);
                m_Pivot.rotation = Quaternion.Lerp(m_Pivot.rotation, rollRotation, m_TurnSpeed * m_CurrentTurnAmount * deltaTime);
            }
        }

        private void ClampPortalFrustum(Portal portal) {
            Vector3 p1 = m_Cam.position;
            Vector3 p2 = m_Pivot.position;
            Vector3 direction = (p1 - p2).normalized;
            float enter = 0;
            if (portal.Plane.Raycast(new Ray(p2, direction), out enter)) {
                Vector3 intersection = p2 + enter * direction;

                Vector3 localIntersection = portal.transform.InverseTransformPoint(intersection);
                Vector3 clampedLocalIntersection = Vector3.Min(new Vector3(0.5f, 0.5f, 0.0f) * m_PortalRigidity, Vector3.Max(new Vector3(-0.5f, -0.5f, 0.0f) * m_PortalRigidity, localIntersection));
                Vector3 clampedWorldIntersection = portal.transform.TransformPoint(clampedLocalIntersection);

                Vector3 newCameraForward = p2 - clampedWorldIntersection;
                Quaternion rotation = Quaternion.LookRotation(newCameraForward, Vector3.up);
                Debug.DrawLine(p1, p2);
                Debug.DrawLine(clampedWorldIntersection, p2, Color.cyan);
                //UnityEditor.EditorApplication.isPaused = true;
                m_Pivot.rotation = Quaternion.Lerp(m_Pivot.rotation, rotation, m_PortalClampSpeed* m_CurrentTurnAmount * Time.deltaTime);
            }
        }
    }
}
